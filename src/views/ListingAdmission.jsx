...
// Column config (matches your table)
const COLUMNS = [
  { key: "id", label: "ID" },
  { key: "name", label: "Name" },
  { key: "fatherName", label: "Father's Name" },
  { key: "dob", label: "DOB" },
  { key: "courseAppliedFor", label: "Course" },
  { key: "village", label: "Village" },
];

// Create the same preview markup you had in the HTML (includes ALL fields)
    ...
    <div id="preview-content-inner">
      <div class="p-8">
        <div class="text-center mb-8">
          <h2 class="text-2xl font-bold text-indigo-800">Admission Form</h2>
          <h3 class="text-3xl urdu font-bold">فارمِ داخلہ</h3>
        </div>
 ...
 * ListingAdmission
 * Props:
 * - data: optional array of admission rows. If omitted, seeds from the HTML sample.
 * - onCreateNew: function to navigate/show AddAdmission (called when "Create New" is clicked).
 */
export default function ListingAdmission({ data: rowsProp = [], onCreateNew }) {
      const navigate = useNavigate();
  // seed like original HTML if no prop provided
  const seed = useMemo(
    () =>
      rowsProp.length
        ? rowsProp
        : [
            {
              ...
              presentAddress: "",
              phone: "9876543210",
              guardianContact: "Ahmed Ali / 9876543210",
              previousSchool: "Al-Hikmah",
              previousQualification: "10th / 2023",
              courseAppliedFor: "Aalia",
              skills: "Urdu, Arabic",
              studentSignature: "M. Ali",
              guardianSignature: "A. Ali",
  ...
  // Actions — all include the entire entry
  ...
  function handlePrint(row) {
    const html = generatePreviewHTML(row);
    printHTML(html); // prints ALL fields
  }
  function handleDownloadPdf(row) {
    const html = generatePreviewHTML(row);
    exportPdf(html, `admission-form-${row.id}.pdf`); // ALL fields in the PDF
  }
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm">
                    <div className="flex flex-wrap items-center gap-2">
                      <button
                        ...
                        className="cursor-pointer inline-flex items-center gap-2 px-3 py-1.5 rounded-lg border border-indigo-200 text-indigo-700 hover:bg-indigo-50"
                      ...
                      <button
                        ...
                        className="cursor-pointer inline-flex items-center gap-2 px-3 py-1.5 rounded-lg border border-green-200 text-green-700 hover:bg-green-50"
                      ...
                      <button
                        ...
                        className="cursor-pointer inline-flex items-center gap-2 px-3 py-1.5 rounded-lg border border-red-200 text-red-700 hover:bg-red-50"
